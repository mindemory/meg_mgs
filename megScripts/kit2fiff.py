...
event_id = {'fixation':161, 'sample':162, 'delay':163, 'response':164, 'feedback':165, 'iti':166} # use event ids from experiment
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith('NR.sqd') and 'emptyroom' not in filename.lower(): #process only noise reduced signal
                ...
                command = [
                    ...
                    '--stim', '161:165', #stim channel naming convention, might break
                    '--slope','-',
                    '--stimthresh', '1',
                ]
...
bidsRoot = os.path.join(os.sep, 'd', 'DATD', 'datd', 'MEG_MGS', 'MEG_BIDS', 'derivatives')
taskName = 'mgs'
...
nruns = 11
run = 1
fName_root = f'sub-{subDirs[sidx][-2:]}_task-{taskName}_'#run-{run:02d}_'

base_path = os.path.join(bidsRoot, subDirs[sidx], 'meg')
# bids_root = os.path.join(base_path, 'fif')
process_all_folders(base_path, fName_root)