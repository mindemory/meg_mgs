...
def load_source_space_data(subjID, bidsRoot, taskName, voxRes, targetLoc):
...
def compute_connectivity_measures(seed_indices, data_matrix, time_vector, connectivityType, batch_size=500):
    """
    ...
    window_samples = int(0.333 * sfreq)  # 333ms window in samples
...
def main(subjID, voxRes, seedROI, targetLoc, connectivityType):
    ...
    outputFile = os.path.join(outputDir, f'sub-{subjID:02d}_task-{taskName}_seededConnectivity_{voxRes}_{seedROI}_{targetLoc}_{connectivityType}.pkl')

    if not os.path.exists(outputFile):
        print('Loading source space data for connectivity analysis...')
        data_matrix, time_vector = load_source_space_data(subjID, bidsRoot, taskName, voxRes, targetLoc)
        ...
        if seedROI == 'left_visual':
            connectivity_measure = compute_connectivity_measures(left_visual_indices, data_matrix, time_vector, connectivityType)
        elif seedROI == 'right_visual':
            connectivity_measure = compute_connectivity_measures(right_visual_indices, data_matrix, time_vector, connectivityType)
        elif seedROI == 'left_frontal':
            connectivity_measure = compute_connectivity_measures(left_frontal_indices, data_matrix, time_vector, connectivityType)
        elif seedROI == 'right_frontal':
            connectivity_measure = compute_connectivity_measures(right_frontal_indices, data_matrix, time_vector, connectivityType)
        else:
...
if __name__ == '__main__':
    ...
    main(subjID, voxRes, seedROI, targetLoc, connectivityType)
