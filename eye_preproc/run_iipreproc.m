function [ii_data,ii_cfg,ii_sacc] = run_iipreproc(edf_fn,cfg_fn,preproc_fn,ii_params,trialinfo,stiminfo, skip_steps)
...
if nargin < 8 || isempty(skip_steps)
...
f_firstpass = ii_plottimeseries(ii_data,ii_cfg,{'X','Y','TarX', 'TarY'},'nofigure');
saveas(f_firstpass,sprintf('%s_raw_timeseries.png',preproc_fn),'png');
% Correct for blinks
...
f_blinkcorrect = ii_plottimeseries(ii_data,ii_cfg,{'X','Y','TarX', 'TarY'},'nofigure');
saveas(f_blinkcorrect,sprintf('%s_blnkcorrect_timeseries.png',preproc_fn),'png');
